...
uint8_t DS3231::triggeredAlarm1() { return (readRegister(REG_STATUS) >> BIT_A1F) & 1; }
...
uint8_t DS3231::triggeredAlarm2() { return (readRegister(REG_STATUS) >> BIT_A2F) & 1; }
 ...
 *
 * This method forces the temperature sensor to convert the temperature into digital code and
 * execute the TCXO algorithm to update the capacitance array of the oscillator.
 *
 ...
 *
 * This method reads the temperature from the temperature registers in degrees Celsius. Bear in mind
 * that these registers are updated every 64-seconds or after forceTemperatureUpdate() is called.
 * Therefore, if you try to call this method multiple times, within 64-seconds (without forcing
 * temperature update), the result will be the same.
 *
