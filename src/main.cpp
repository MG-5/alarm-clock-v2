        ...
        switch (currentState)
        {
            case States::CLOCK:
                if ((millis() - prevTime) >= 1000)
                {
                    ...
                    if (!night && t.hour >= 0 &&
                        t.hour <= 3) // Nacht-Modus inaktiv & Zeit zum Abschalten?
                    {
                        setNightmode(true);
                        saveStatistics();
                    ...
                    if (counter <= 15 && alarmState != AlarmStates::ALARM1_VIBR &&
...
#pragma region Alarmtriggers
        if (check_bit(interruptFlags, 3))
        {
            ...
            {
                t = rtc.getTime();
                if (alarmMode == AlarmModes::ALARM1_ACTIVE ||
                    alarmMode == AlarmModes::ALARM1_ALARM2_ACTIVE)
                {
                    A1_triggers++;

                    if (alarmState != AlarmStates::ALARM2_LED &&
                        alarmState != AlarmStates::ALARM2_VIBR)
                    {
            ...
            {
                t = rtc.getTime();
                if (alarmMode == AlarmModes::ALARM2_ACTIVE ||
                    alarmMode == AlarmModes::ALARM1_ALARM2_ACTIVE)
                {
                    A2_triggers++;
    ...
    switch (currentState)
    {
        ...
        default:
            if (currentState != States::CHANGE_ALARM1_MINUTE &&
                ...
                currentState != States::CHANGE_ALARM2_HOUR && currentState != States::TEST)
            {
                currentState = States::CLOCK;
                counter = 0;
                prevTime = 0;
